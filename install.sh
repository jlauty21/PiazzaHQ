...
# --remote-debugging-port/--remote-allow-origins: exposes the Chrome DevTools
# Protocol for manual debugging (telling a genuinely frozen kiosk tab apart
# from a browser process that's merely still alive, or just inspecting the
# live page). Binds to loopback only when no --remote-debugging-address is
# given — never reachable off this Pi.
